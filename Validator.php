 ...
 * the Validator object generates an array of missing items.
 *
...
class Validator
{
    ...
    /**
     * Checks whether the date is submitted with the correct pattern:
     * YYYY-MM-DD (MySQL Format) and has valid date parts.
     *
     * @param string $fieldName Name of submitted value to be checked.
     * @param mixed $pattern Regex
     * @param string $date Date to be checked.
     * @throws Exception Incorrect format or date part(s).
     */
    public function checkDate($fieldName, $pattern, $date)
    {
        ...
        if (strpos($date, '-')) {
            if (is_numeric($date_array[0]) &&
                ...
                // The checkdate function has the parameter order month, day, year
                // The year is between 1 and 32767.
                // The number of days for the given month and leap years are taken into consideration.
                if (!checkdate($date_array[1], $date_array[2], $date_array[0])) {
                    $this->_errors[$fieldName] = $fieldName . ': The date is not valid!';
                }
            }
        } else {
            $this->_errors[$fieldName] = $fieldName . ': The date is not valid!';
