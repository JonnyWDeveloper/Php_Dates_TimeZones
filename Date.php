...
class Date extends DateTime
{
     ...
     * @param Date $endDate Finishing date.
     * @return int Number of days between start and end dates.
     */
    static public function dateDiff(DateTime $startDate, DateTime $endDate)
    {
    ...
    /**
     * Static method to calculate the number of hours between two dates.
     *
     * @param DateTime $startDate
     * @param DateTime $endDate
     * @return float|int
     */
    static public function hourDiff(DateTime $startDate, DateTime $endDate)
    {
        $start = gmmktime(0, 0, 0, $startDate->format('m'), $startDate->format('d'), $startDate->format('Y'));
        $end = gmmktime(0, 0, 0, $endDate->format('m'), $endDate->format('d'), $endDate->format('Y'));
    ...
    {
        throw new Exception ('modify($modify) { has been disabled.');
    }
    ...
    public function setMDY($USDate)
    {
        ...
        if (!is_array($dateParts) || count($dateParts) != 3) {
            throw new Exception ('setMDY() expects a date as "MM/DD/YYYY Or MM-DD-YYYY Or MM.DD.YYYY Or MM:DD:YYYY Or MM DD YYYY" .');
        }
     ...
     * passes them to the overridden setDate() method.
     *
     * @param string $EuroDate Date string in European DD/MM/YYYY format.
     * @throws Exception
     */
    public function setDMY($EuroDate)
    {
        ...
        if (!is_array($dateParts) || count($dateParts) != 3) {
            throw new Exception ('setMDY() expects a date as "MM/DD/YYYY Or MM-DD-YYYY Or MM.DD.YYYY Or MM:DD:YYYY".');
        }
        $this->setDate($dateParts [2], $dateParts [1], $dateParts [0]); //YYYY-MM-DD.
    }
     ...
     *
     * @param Date $endDate Date that is to be compared with object.
     * @return int Number of days between both dates.
     */
    public function dateDiff2(Date $endDate)
    {
